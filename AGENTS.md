...
- **Backend:** интеграционные тесты на xUnit + `WebApplicationFactory` + Testcontainers (PostgreSQL/MongoDB/MinIO). Моки БД не использовать — тесты гоняются против реальных контейнеров.
- **E2E:** ключевые сквозные сценарии (зона R из `docs/5/test_coverage_scope.md`) — Playwright.
- **Frontend unit-тесты не пишутся** — низкий ROI для дипломной работы.
- Скоуп покрытия и приоритеты — см. `docs/5/test_coverage_scope.md`. Минимум: 100 % P1-кейсов проходят зелёными.
- Тесты лежат в `backend/tests/` и `frontend/e2e/` (не в `src/`).
...
├── .gitignore
├── README.md
└── AGENTS.md                   — правила написания кода
```
